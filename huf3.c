...
btree* create_btree(byte c, long long int freq, btree *left, btree *right)
{
  ...
  void *f = (void*) &freq;
  new_btree-> frequence = (long long int *)f;
...
btree *building_huff_tree(heap *hp, long long int t)
{
  ...
  void *s = left->frequence + right->frequence;

  long long int freq = (long long int *)s;
...
int main()
{
  ...
  return 0; // retornar 0 significa que a operacao foi bem sucedida//saudades da bahia!
  }
